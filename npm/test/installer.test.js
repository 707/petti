...
test("versionTagFor prefixes versions with v", () => {
  assert.equal(versionTagFor("0.6.3"), "v0.6.3");
  assert.equal(versionTagFor("v0.6.3"), "v0.6.3");
});

test("assetNameFor builds the release archive name", () => {
  assert.equal(assetNameFor("0.6.3", "darwin", "amd64"), "petti_0.6.3_darwin_amd64.tar.gz");
  assert.equal(assetNameFor("0.6.3", "linux", "arm64"), "petti_0.6.3_linux_arm64.tar.gz");
});

test("releaseURLFor points at the GitHub release asset", () => {
  assert.equal(
    releaseURLFor("0.6.3", "darwin", "arm64"),
    "https://github.com/707/petti/releases/download/v0.6.3/petti_0.6.3_darwin_arm64.tar.gz",
  );
});

test("parseChecksums reads release checksum files", () => {
  const checksums = parseChecksums([
    "aaa  petti_0.6.3_darwin_amd64.tar.gz",
    "bbb  petti_0.6.3_darwin_arm64.tar.gz",
  ].join("\n"));

  assert.equal(checksums.get("petti_0.6.3_darwin_amd64.tar.gz"), "aaa");
  assert.equal(checksums.get("petti_0.6.3_darwin_arm64.tar.gz"), "bbb");
});
